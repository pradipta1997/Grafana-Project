                    ...
                    <table id="tableBarang" class="table table-bordered table-striped" style="width: 100%;">
                        <thead style="background-color: #012a4a; color: white;">
                            <tr role="row">
                                <th>No</th>
                                <th>Jumlah</th>
                                <th>Update Data Alat Alarm System Acces Door</th>
                                <th>User</th>
...
<script>
    const urlUpdatedataAlatAlarmSystemDanAccesDoor = '<?= site_url("UpdatedataAlatAlarmSystemDanAccesDoor/") ?>';
    let table;
    ...
    $(function() {
        if (!$.fn.DataTable.isDataTable('#tableBarang')) {
            table = $('#tableBarang').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: urlUpdatedataAlatAlarmSystemDanAccesDoor + "listUpdateDataAlatAlarmSystemAccesDoor",
                    type: "POST"
                },
                columnDefs: [{
                    targets: [0, -1],
