...
<script>
    const urlRekapDataPerformanceClusterBulanan = '<?= site_url("RekapDataPerformanceClusterBulanan/") ?>';
    let table;
    ...
    $(function() {
        if (!$.fn.DataTable.isDataTable('#tableBarang')) {
            table = $('#tableBarang').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: urlRekapDataPerformanceClusterBulanan + "listRekapDataPerformanceClusterBulanan",
                    type: "POST"
                },
                columnDefs: [{
                    targets: [0, -1],
