...
<script>
    const urlGrafikProgressdanPendingCaseCabang = '<?= site_url("GrafikProgressdanPendingCaseCabang/") ?>';
    let table;
    ...
    $(function() {
        if (!$.fn.DataTable.isDataTable('#tableBarang')) {
            table = $('#tableBarang').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: urlGrafikProgressdanPendingCaseCabang + "listGrafikProgressdanPendingCaseCabang",
                    type: "POST"
                },
                columnDefs: [{
                    targets: [0, -1],
