...
<script>
    const urlUpdateKategoriKasusClosedCaseNominal = '<?= site_url("UpdateKategoriKasusClosedCaseNominal/") ?>';
    let table;
    ...
    $(function() {
        if (!$.fn.DataTable.isDataTable('#tableBarang')) {
            table = $('#tableBarang').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: urlUpdateKategoriKasusClosedCaseNominal + "listUpdateKategoriKasusClosedCaseNominal",
                    type: "POST"
                },
                columnDefs: [{
                    targets: [0, -1],
