                    ...
                    <table id="tableBarang" class="table table-bordered" style="width: 100%;">
                        <thead>
                            <tr role="row">
                                ...
                                <th>Kantor Cabang</th>
                                <th>Jumlah ATM</th>
                                <th>Reability Sebelum Sanggahan (%)</th>
                                <th>Tagihan Invoice Sebelum Sanggahan (%)</th>
                                <th>User</th>
...
<script>
    const urlPropenreabbribul = '<?= site_url("Propenreabbribul/") ?>';
    let table;
    ...
    $(function() {
        if (!$.fn.DataTable.isDataTable('#tableBarang')) {
            table = $('#tableBarang').DataTable({
                ...
                scrollX: true,
                ajax: {
                    url: urlPropenreabbribul + "listPropenreabbribul",
                    type: "POST"
                },
                columnDefs: [{
                    targets: [0, -1],
