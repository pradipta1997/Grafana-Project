...
class Tlatmbri562 extends MY_Controller
{
    ...
    public function index()
    {
        ...
        // cekPergroup();
        $this->header('Timeline ATM BRI 562');
        $this->load->view('CHM/list_Tlatmbri562', $data);
        $this->footer();
    }

    public function listTlatmbri562()
    {
        ...
        foreach ($list as $results) {
            ...
            $row[] = "<a href='" . base_url("Tlatmbri562/editTlatmbri562/" . $results->NO) . "' class='btn-warning    
                        btn-sm' " . getEditperm() . ">
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Serverside->_countAll('tbl_timeline_ho_atm_bri_562'),
            "recordsFiltered" => $this->Serverside->_serverSide('tbl_timeline_ho_atm_bri_562'),
            "data" => $data,
        );
    ...
    public function editTlatmbri562($NO)
    {
    	$data = array (
    		'NO_TL' => $this->General->getRow('tbl_timeline_ho_atm_bri_562',['NO' => $NO])
    	);
    	// cetak_die($data);
    	$this->header('Timeline ATM BRI Kolaborasi');
        $this->load->view('Tlatmbri562/edit_Tlatmbri562', $data);
        $this->footer();
    }

    public function updateTlatmbri562($NO)
    {
    	$data = array(
    		'NO' => $NO,
    	...
    	// cetak_die($data);
    	$update = $this->General->update_record($data, ['NO' => $NO], 'tbl_timeline_ho_atm_bri_562');
    	// lastq();
    	 if ($update) {
