...
class Tlatmbrikolab extends MY_Controller
{
    ...
    public function listTlatmbrikolab()
    {
        ...
        foreach ($list as $results) {
            ...
            $row[] = "<a href='" . base_url("Tlatmbrikolab/editTLatmbrikolab/" . $results->NO) . "' class='btn-warning    
                        btn-sm' " . getEditperm() . ">
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Serverside->_countAll('tbl_timeline_ho_atm_bri_kolaborasi'),
            "recordsFiltered" => $this->Serverside->_serverSide('tbl_timeline_ho_atm_bri_kolaborasi'),
            "data" => $data,
        );
    ...
    public function editTLatmbrikolab($NO)
    {
    	$data = array (
    		'NO_TL' => $this->General->getRow('tbl_timeline_ho_atm_bri_kolaborasi',['NO' => $NO])
    	);
    	// cetak_die($data);
    	$this->header('Timeline ATM BRI Kolaborasi');
        $this->load->view('TLatmbrikolab/edit_TLatmbrikolab', $data);
        $this->footer();
    }

    public function updateTlatmbrikolab($NO)
    {
    	$data = array(
    		'NO' => $NO,
    	...
    	// cetak_die($data);
    	$update = $this->General->update_record($data, ['NO' => $NO], 'tbl_timeline_ho_atm_bri_kolaborasi');
    	// lastq();
    	 if ($update) {
